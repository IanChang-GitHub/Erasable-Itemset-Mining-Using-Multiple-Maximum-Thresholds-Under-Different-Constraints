...
public class Find_E1
{
	...
	//-------------------------------
	//取E1的使用的記憶體
	//static	long useMemory; 		//宣告未使用前記憶體狀況與使用後記憶體狀況，兩者相減就是使用記憶體量！
	//static  int diffmemory_int;
	static	Runtime r = Runtime.getRuntime(); 	//這是宣告相關參數
	//static	String usedStr;   			        //列印「使用記憶體量」字串
			
	public Find_E1(char[][] dbData, int[] transProfit, double minGain, double totalGain) //建構子
	{
		try
		{		
			//------------------------------------------
			// 設定「初始值」
			this.minGain = minGain;//門檻
    ...
    public void Find_E1()
    {
		try
		{
			...
			for(int i=0;i<dbData.length;i++) //計算1-itemset gain
			{						
