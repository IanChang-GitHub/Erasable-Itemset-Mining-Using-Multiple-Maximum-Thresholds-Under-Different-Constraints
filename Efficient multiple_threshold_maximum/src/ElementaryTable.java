...
public class ElementaryTable {
	...
	public int hashCode(char[] key) {
    	...
    	public HashSet<Integer> getpidSet(char key) {
    		char[] k = new char[1];
        	k[0]= key; 
        	int hash = hashCode(k);
        	int i = indexFor(hash, table.length);
	...
	public void add(String str, int value) {
    	...
    	public void add(char ch, int value, int pid) {
    		char[] k = new char[1]; //1-item
    		k[0]=ch;
    		this.add(k, value, pid);
    	}
	...
	// 初始化每個itemset的value
    	...
    	public void add(char[] key, int value, int pid) {
        	char[] k = key; //itemset
        	int hash = hashCode(k); //計算hash值
        	int i = indexFor(hash, table.length); //根據hash值計算儲存位子
        	...
        	pidset.add(pid);
        	addElement(hash, k, value, i, pidset);
        	//table[i].pidset.add(pid);
        	return;
    	}
    	
    	public void add(char[] key, int value, HashSet pidset) {
        	char[] k = key; //itemset
        	int hash = hashCode(k); //計算hash值
        	int i = indexFor(hash, table.length); //根據hash值計算儲存位子
...
/*	
  ...
  */
    	...
    	void addElement(int hash, char[] key, int value, int bucketIndex, HashSet<Integer> pidset) {
        	table[bucketIndex] = new Element(hash, key, value, table[bucketIndex]);
        	table[bucketIndex].pidset = pidset;
        	if (size++ >= threshold) 
            		resize(2 * table.length);
    	}
