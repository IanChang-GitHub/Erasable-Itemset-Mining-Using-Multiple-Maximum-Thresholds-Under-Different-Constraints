...
public class Find_E2 {
	...
	public Find_E2(char[][] dbData, int[] transProfit, double[] maxGainThreshold, double totalGain, ElementaryTable E1, ElementaryTable C1)
	{
		try{
			//------------------------------------------
			// 設定「初始值」
	...
	*/
	...
	{		
		try{			
			...
			char[] C1_arr = new char[C1.size()]; //Erasable 1-itemset
			int pos=0;
				
			Set set0 = C1.ElementSet();
 			Iterator iter0 = set0.iterator();
 			Element nx0;
 			while (iter0.hasNext())    
 			{
 	   		    nx0 = (Element)iter0.next();
				C1_arr[pos] = nx0.getKey()[0];
 			...
 			qsi.put(C1_arr,maxGainThreshold);
 			/*
	...
	*/			
			...
			for(int i=0;i<C1_arr.length-1;i++)
			{
				//System.out.println(C1.getValue(C1_arr[i])+"  "+maxGainThreshold[C1_arr[i]]);
				if(C1.getValue((char)C1_arr[i]) <= maxGainThreshold[C1_arr[i]])
				{
					for(int j=i+1;j<C1_arr.length;j++)
					{
						if (C1.getValue(C1_arr[j]) <= maxGainThreshold[C1_arr[i]])
						{
			...
			Set set3 = C2.ElementSet();
     		...
     		while (iter3.hasNext())    
 			{
				...
				double gainMaxConstraint = maxGainThreshold[nx3.getKey()[0]];
				...
				for (int k=0; k < nx3.getKey().length; k++)
   			    {
   			        ...
   			        /*
   			        if(maxGainThreshold[nx3.getKey()[k]] > gainMaxConstraint)
   			        {
			        	gainMaxConstraint = maxGainThreshold[nx3.getKey()[k]];
   			        }
   			        */
   			        gainAvgConstraint = gainAvgConstraint+maxGainThreshold[nx3.getKey()[k]];
   			        //System.out.println("Kept_Items.size()" + Kept_Items.size());
   			    }						    
				gainAvgConstraint = gainAvgConstraint/nx3.getKey().length;
