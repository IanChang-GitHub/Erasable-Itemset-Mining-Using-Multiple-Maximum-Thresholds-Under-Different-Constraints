...
public class Find_ErasableItemsets
{
	...
	public Find_ErasableItemsets(char[][] dbData, int[] transProfit, double maxGainThreshold[],  double totalGain, ElementaryTable E1, ElementaryTable E2,ElementaryTable C1, ElementaryTable C2, int maxLength)
    {
	      try{			
				    //---------------------------
				    //宣告變數區
				    ...
				    Current_Len = 3; //設定長度為 3
 				    ...
 				    if(CI[0].size()==0 || CI[1].size()==0)return;
				    ...
				    {					
					      ...
					      EI[Current_Len-1] = new ElementaryTable();
					      Sort_C Sort_CI = new Sort_C(CI[Current_Len-2]);//先進行排序動作！(k-1的erasable itemset)

					      // generate the new candidates k-itemsets
					      Gen_C Gen_AC = new Gen_C(Sort_CI.get_Sort_CItemsets(), CI[Current_Len-2], Current_Len, maxGainThreshold);					
					
					      // save the new candidates k+1-itemsets in the EI tables
					      CI[Current_Len-1] = Gen_AC.get_CItemsets();
					     ...
					     if(CI[Current_Len-1].size()==0)
					     {
					    	 System.out.println ("Candidate "+Current_Len+"-itemset is empty");
						       break;
					     }
					     ...
					     /*
					     if(CI[Current_Len-1].size()<Current_Len)
					     {
					    	 System.out.println ("The number of candidate "+Current_Len+"-itemset < " +Current_Len);
						       break;
					     }	
					     */			
					     Current_Len++;					
    ...
    //--------------------------------------------------
	  ...
	  {		
		    try{							   
       			...
       			Set set = CI[Current_Len-1].ElementSet(); 
       			Iterator iter = set.iterator();
       			for (int j=0; iter.hasNext(); j++) 
       			{       					
       			    Kept_Items.clear();
       			    Element nx=(Element)iter.next();
       			    //double gainMaxConstraint = 0;
       			    double gainMaxConstraint = maxGainThreshold[nx.getKey()[0]];
       			    double gainAvgConstraint = 0;
       			    ...
       			    for (int k=0; k < nx.getKey().length; k++)
       			    {
       			        ...
       			        /*
       			        if(maxGainThreshold[nx.getKey()[k]] > gainMaxConstraint){
 			        	gainMaxConstraint = maxGainThreshold[nx.getKey()[k]];
    			        }
    			        */
    			        gainAvgConstraint = gainAvgConstraint+maxGainThreshold[nx.getKey()[k]];
       			        //System.out.println("Kept_Items.size()" + Kept_Items.size());
       			    }
       			    gainAvgConstraint = gainAvgConstraint/nx.getKey().length; //取平均
       			    //2.在產品資料庫，檢查每筆產品內的原料是否有存在Kept_Items，有的話，該筆產品的利潤值為該itemset的gain值
					      ...
					      {						
						        ...
						        {																
									...
									{											                            
										CI[Current_Len-1].add(nx.getKey(), transProfit[m]);                            
			              ...
			              System.out.print(" gain: "+ nx.getValue());
			              System.out.print(" Average: "+ gainAvgConstraint);
			              System.out.println(" Maximum: "+ gainMaxConstraint);
			              */
  						  if(gain<=gainMaxConstraint)
  						  {
  							  if (gain<=gainAvgConstraint)
  							  {
  								EI[Current_Len-1].add(nx.getKey(), gain);
  							  }
  						  }
  						  else
  						  {
  							    CI[Current_Len-1].remove(nx.getKey());
					      }					      		
       			}          			
       			r.gc();
	  ...
	  {		
		    long t1 = System.currentTimeMillis();	
			  ...
			  CI = new ElementaryTable[maxLength];
