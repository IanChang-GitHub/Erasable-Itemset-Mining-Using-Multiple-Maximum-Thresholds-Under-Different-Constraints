...
public class FPGrowth {
    ...
    private FPtree tree_clone(FPtree tree, Vector<FPtree> headerTable) {   //複製EPtree
    	FPtree treeClone = new FPtree("null");
    	treeClone.item = null;
    	treeClone.root = true;
    	node_clone(tree, headerTable, treeClone);
    ...
    private void node_clone(FPtree tree, Vector<FPtree> headerTable, FPtree treeClone) {  
    	//FPtree treeClone = new FPtree("null");
    	//treeClone.item = null;
    	//treeClone.root = true;
        //remember our transactions here has oredering and non-frequent items for condition items
    	...
    	for (FPtree child:tree.children) {
    		//treeClone.children.add(new FPtree(child.item));
    		FPtree node = new FPtree(child.item);
    		node.parent = treeClone;
    		node.count = child.count;
    		node.root = child.root;
    		
    		for (FPtree headerPointer : headerTable) {
                if (headerPointer.item.equals(node.item)) {
                    while (headerPointer.next != null) {
                        headerPointer = headerPointer.next;
                    }
    		...
    		treeClone.children.add(node);
    		node_clone(child, headerTable, node);
    	}
    }
    
    private Vector<FPtree> headerTable_clone (Vector<FPtree> headerTable) { //複製headerTable
    	 Vector<FPtree> headerClone = new  Vector<FPtree>();
    	 for(FPtree headerPointer:headerTable) {
    		 FPtree item = new FPtree(headerPointer.item);
    		 item.next = null;
    		 item.count = headerPointer.count;
    		 headerClone.add(item);
    	 }
    ...
    private void fpgrowth(FPtree fptree, Map<String, Double> threshold, Vector<FPtree> headerTable) {
        ...
        FPgrowth(fptree, null, 0, headerTable, frequentPatterns);
    }

    void FPgrowth(FPtree fptree, String base, int baseGain, Vector<FPtree> headerTable, Map<String, Integer> frequentPatterns) {
        if (fptree.children.isEmpty() == false)
        {
	    	for (FPtree iteminTree : headerTable) 
	        {
	        	int itemIndex = headerTable.indexOf(iteminTree); 
	        	...
	        	*/
	            ...
	            System.out.println("=================================");
	            if (gainofCurrentPattern <= MGT.get(iteminTree.item))
	            {
	            	...
	            	Vector<FPtree> headerClone = headerTable_clone (headerTable); //複製標頭
	            	//Collections.sort(headerClone, new frequencyComparitorinHeaderTable(MGT)); //排序
	            	FPtree treeClone = tree_clone(fptree, headerClone); //複製eptree
	            	FPtree item = headerClone.get(itemIndex);
	            	
	            	if (!iteminTree.item.equals(headerClone.firstElement().item)) //刪除排在header table後面item的所有結點
	            	{
	            		int i = 0;
	            		while(!headerClone.get(i).item.equals(iteminTree.item))
	            		{
	            			FPtree deleteNode = headerClone.get(i);
	            			while (deleteNode.next != null)
	            			{
	            				deleteNode = deleteNode.next;
	            				deleteNode.parent.children.removeElement(deleteNode);
	            			}
	            			headerClone.get(i).next = null;
	            			i++;
	            		}
	            	}
	            	...
	            	{
	            		item = item.next;
	            		FPtree parentPointer = item;
	            		FPtree zeroGain = item;
	            		while(parentPointer.parent.isRoot() != true)
	            		{
	            			parentPointer = parentPointer.parent;
	            			parentPointer.count = parentPointer.count - item.count;
	            			
	            			if (parentPointer.count == 0) {
	            				zeroGain = parentPointer;
	            			}
	            			
	            		}
	            		zeroGain.parent.children.removeElement(item);
	            		//item.parent.children.removeElement(item);
	            	}
	            	...
	            	if (!treeClone.children.isEmpty()) {
	                    FPgrowth(treeClone, currentPattern, gainofCurrentPattern, headerClone, frequentPatterns);
	                }
	            	*/
	            	FPgrowth(treeClone, currentPattern, gainofCurrentPattern, headerClone, frequentPatterns, MGT.get(iteminTree.item));
	            }
    ...
    void FPgrowth(FPtree fptree, String base, int baseGain, Vector<FPtree> headerTable, Map<String, Integer> frequentPatterns,double mgt) {
        if (fptree.children.isEmpty() == false)
        {
	    	for (FPtree iteminTree : headerTable) 
	        {
	        	...
	        	*/
	            ...
	            if (gainofCurrentPattern <= mgt)
	            {
	            	...
	            	*/
	            	FPgrowth(treeClone, currentPattern, gainofCurrentPattern, headerClone, frequentPatterns,mgt);
	            }
