...
public class Erasable_function 
{
    ...
    static	long freeMemory, totalMemory, endMemory; 		//宣告未使用前記憶體狀況與使用後記憶體狀況，兩者相減就是使用記憶體量！
	...
	static ElementaryTable C1 ;
	static ElementaryTable C2 ;
	...
	public static double getUpperThreshold(double threshold)
	{           
	...
	public static String getMultiThreshold(String filepath2)
	{
		filePath2=filepath2;
		return filePath2;
	}
	...
	public static void main(String[] args)
    {
        ...
        {	
			...
			System.out.println("Erasable Itemset Mining Algorithm with function constraint");
 			System.out.println("=====================================");
			//System.out.println("minGain: "+ minGain);
 			filePath=getDatabase("item-6"); //設定資料庫檔案路徑
 			filePath2=getMultiThreshold("t");//設定門檻值檔案路徑
 			upperThreshold = getUpperThreshold(0.2);
			...
			upperGain = totalGain * upperThreshold;
			System.out.println("-----------------------------");
			...
			Find_E1  fe1 = new Find_E1(dbData, transProfit, maxGainThreshold, upperGain); //mining erasable 1-itemsets(product,profit,threshold,總profit)
			...
			if(C1.size()!=0)
			{
				...
				Find_E2  fe2 = new Find_E2(dbData, transProfit, maxGainThreshold, upperGain, C1);
				E2 = fe2.getE2();
				C2 = fe2.getC2();
				...
				*/
				...
				Find_ErasableItemsets efi = new Find_ErasableItemsets(dbData, transProfit, maxGainThreshold, upperGain, E1, E2, C1, C2, maxLength);
			...
			usedStr = String.valueOf((r.totalMemory() - r.freeMemory())/1024)+ "KB used " +  String.valueOf((r.totalMemory() - r.freeMemory())/1024/1024) +"MB used"   ;  
