...
public class Find_ErasableItemsets
{
	  ...
	  int CK_Num =0;
	  int EI_Num =0;
	  long temp_memory, memory = 0;
	
	public Find_ErasableItemsets(char[][] dbData, int[] transProfit, double maxGainThreshold[],  double upperGain, ElementaryTable E1, ElementaryTable E2,ElementaryTable C1, ElementaryTable C2, int maxLength)
    {
	      try{			
				    //---------------------------
				    //宣告變數區
				    ...
				    {					
					      ...
					      Gen_C Gen_AC = new Gen_C(Sort_CI.get_Sort_CItemsets(), CI[Current_Len-2], Current_Len);					
	  ...
	  */
	  ...
	  {		
		    try{							   
				...
				Cal_Function function = new Cal_Function();								
       			...
       			{       					
       			    ...
       			    double Fun_Constraint = function.calculate_avg(nx.getKey(), maxGainThreshold, upperGain);
       			...
       			if(temp_memory > memory)
       				memory = temp_memory;    
       			//tempMemory = r.totalMemory() - r.freeMemory();
